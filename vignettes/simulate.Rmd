...
lm_const=0.5
lm_rate=0.5
plot(c(0,3),lm_const+c(0,3)*lm_rate,xlab='Time since infection',
     ylab='Within host effective population size',type='l',ylim=c(0,lm_const+3*lm_rate))
```
...
simu <- sim_ongoing(pi=pi,off.r=off.r,w.shape=w.shape,
                    w.scale=w.scale,dateStartOutbreak=2005,dateT=dateT,add.prob = 0.6,
                    lm_const = lm_const,lm_rate = lm_rate)
```
...
Typically inference would start with a phylogenetic tree stored in a Newick file.
We also need to know the date of the last sample, in order to know the absolute dates of
all nodes in the tree. Let's write the tree from the simulation above to a Newick file,
read it and combine with the known date of the last sample to produce an absolutely dated tree:
...
```{r,results='hide'}
res<-inferTTreeM(ptree,mcmcIterations=100,w.shape=w.shape,w.scale=w.scale,dateT=dateT,
                 startPi=0.25,updatePi=T,startOff.r=2,updateOff.r=T,start_const=1,start_rate=1,
                 updateconst = T,updaterate=T)
```
